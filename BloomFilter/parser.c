...
char *next_word(FILE *infile, regex_t *word_regex) {
  ...
  wordLength = count; //so i can access the length of the words
  return word;
}

int get_length(void) {
  return wordLength; //used to know how many letters to get from the function
}
